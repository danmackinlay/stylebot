...
1. **Data-leakage circularity** — the detector having seen the styler's training rows. *Fixed by a held-out by-POST split* (`train-voice-clf --holdout-frac/--holdout-posts`); use one shared partition across styler-train / detector-train / eval.
