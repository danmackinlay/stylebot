...
def diff_chunks(before: str, after: str) -> list[tuple[str, str]]:
    """Return (before_chunk, after_chunk) pairs for each changed region.
    ...
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        ...
        if not before_chunk.strip() or not after_chunk.strip():
            ...
            continue
        pairs.append((before_chunk, after_chunk))
    return pairs
    ...
    *,
    ...
    return {
        "messages": [
            ...
            {"role": "user", "content": before_body},
            {"role": "assistant", "content": after_body},
        ],
        "meta": meta,
    }
...
def _capture_and_append(
    *,
    ...
    if whole:
        chunk_pairs = [(before_body, after_body)]
    else:
        chunk_pairs = diff_chunks(before_body, after_body)
    ...
    written: list[dict] = []
    for i, (b, a) in enumerate(chunk_pairs):
